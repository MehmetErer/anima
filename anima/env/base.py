...
class EnvironmentBase(object):
    ...
    def get_significant_name(cls, version, include_project_code=True, include_version_number=True):
        ...
        try:
            ...
            if version.task.project.name in ['Castle']:
                if version.take_name.lower() == 'main':
                    sig_name = sig_name.replace('CP3_001_', 'CP3_')
            if version.task.project.name in ['Wolf Hall']:
                if version.take_name.lower() == 'main':
                    a = version.nice_name.split('_')
                    a.pop(-2)
                    sig_name = 'O_%s' % '_'.join(a[:-1])
            if version.task.project.name in ['Turbulence'] and version.task.type.name == 'Comp':
                if version.take_name.lower() == 'main':
                    a = version.nice_name.split('_')
                    a[-1] = 'IMJ'
                    a[-2] = a[-2].lower()
